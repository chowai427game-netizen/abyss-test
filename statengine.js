...
// 🧮 statengine.js：屬性計算與傷害判定引擎 (v2.0 前後端同構純函數版)
// ==========================================================================
...
(function (exports) {
    ...
    function resetCurrentRunData(account, run, buffs = {}, skillsDb = {}, blueprintsDb = []) {
        ...
        const s = {
            STR: Number(acc.stats.STR) || 0,
            AGI: Number(acc.stats.AGI) || 0,
            VIT: Number(acc.stats.VIT) || 0,
            INT: Number(acc.stats.INT) || 0,
            DEX: Number(acc.stats.DEX) || 0,
            LUK: Number(acc.stats.LUK) || 0
        };
        ...
        if (current.skills && sDb) {
            const jobSkills = sDb[job] || [];
            for (let sName in current.skills) {
                const skLv = current.skills[sName];
                if (skLv <= 0) continue;
    ...
    function applyEquipmentStats(slot, account, run, blueprintsDb) {
        ...
        if (!acc || !acc.equipment || !current) return;
        const equipName = acc.equipment[slot];
        if (!equipName || !bDb) return;

        const blueprint = bDb.find(x => x.name === equipName);
        if (!blueprint || !blueprint.stats) return;

        // 🛠️ 修正 Bug：相容 equipmentStars 與 itemRefines 雙命名
        const starLevel = (acc.equipmentStars && acc.equipmentStars[slot]) || (acc.itemRefines && acc.itemRefines[equipName]) || 0;
        const multiplier = 1 + (starLevel * 0.15);
     ...
     * 計算單件裝備最終屬性 preview
     */
    function calculateEquipmentBonus(equipName, account, blueprintsDb) {
        const acc = account || (typeof accountMeta !== "undefined" ? accountMeta : null);
        const bDb = blueprintsDb || (typeof CRAFTING_BLUEPRINTS !== "undefined" ? CRAFTING_BLUEPRINTS : []);

        const blueprint = bDb.find(b => b.name === equipName);
        if (!blueprint) return {};

        const refineLvl = (acc && acc.itemRefines?.[equipName]) || (acc && acc.equipmentStars?.[equipName]) || 0;
        const multiplier = 1 + (refineLvl * 0.15); // 每 +1 增加 15% 基礎屬性
